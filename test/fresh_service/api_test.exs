    ...
    test "returns parsed response (with struct) from given path" do
      with_mocks([
        ...
        {FreshService.Api, [:passthrough],
         [get!: fn _url, _headers -> json_response([%{id: "id"}], 200) end]}
      ]) do
  ...
  describe "auth_header/2" do
    ...
    test "returns encoded credential headers if credentials given" do
      ...
      assert [{:Authorization, "Basic dXNlcm5hbWU6cGFzc3dvcmQ="}] ==
               auth_header(headers, credentials)
    end
    ...
    test "returns encoded credential headers if credentials given along with existing headers" do
      ...
      assert [Authorization: "Basic dXNlcm5hbWU6cGFzc3dvcmQ=", "Content-Type": "application/json"] ==
               auth_header(headers, credentials)
    end
  end
