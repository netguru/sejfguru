  ...
  describe "import/0" do
    test "with no assets returned from API" do
      with_mocks([
        ...
        assert {:ok, "Freshservice assets imported successfully from 0 pages"} ==
    ...
    test "with 1 page of assets returned from API" do
      with_mocks([
           ...
           all: fn i ->
             case i do
               1 ->
                 {:ok,
                  [
                    %FreshService.Asset{name: "device1", display_id: 1},
                    %FreshService.Asset{name: "device2", display_id: 2}
                  ]}
        ...
        assert {:ok, "Freshservice assets imported successfully from 1 pages"} ==
                 AssetImporter.import()
    ...
    test "with 2 page of assets returned from API" do
      with_mocks([
           ...
           all: fn i ->
             case i do
               ...
               2 ->
                 {:ok,
                  [
                    %FreshService.Asset{name: "device3", display_id: 3},
                    %FreshService.Asset{name: "device4", display_id: 4}
                  ]}
        ...
        assert {:ok, "Freshservice assets imported successfully from 2 pages"} ==
                 AssetImporter.import()
    ...
    test "asset insert returned error" do
      with_mocks([
           ...
           upsert_asset: fn attrs ->
             case attrs.freshservice_id do
               1 ->
                 ...
                 |> Sejfguru.Repo.insert(
                   on_conflict: :replace_all,
                   conflict_target: :freshservice_id
                 )
        ...
        assert {:error, "Freshservice assets import finished with error on page 1"} ==
                 AssetImporter.import()
